...
export class OrdersComponent {
  ...
  constructor(
    ...
    private authService: AuthServiceService, private _dialog:MatDialog
  ) {}
  public length = 10;
  public pageIndex = 1;
  public pageSize = 10;
  ...
  updateOrderStatus(status:string,orderId:string){
    this._backendService.makePostApiCall("user/orderStatus",{status,orderId}).subscribe((res:any)=>{
      this._dialog.open(SuccessDialogComponent,{data:{message:`Order ${status} successfully!`}})
      console.log(res)
      this.getOrder(this.pageSize,this.pageIndex)
    })
  }
